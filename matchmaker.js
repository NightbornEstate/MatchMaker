...
function createEmbed(game) {
    let name = game.name, description = game.description, host = game.host, players = game.players, playernumber = game.positions;
...
function host(msg) {
    if(!user.getUserData(msg.author)) {
        msg.reply("You must setup a profile before joining a game! Use the `.ign` to setup a profile").then((amsg) => {
            msg.delete();
            setTimeout(() => {
                amsg.delete();
            }, 3000);    
    ...
    if(tokens.length < 3) {
        msg.delete();
        msg.reply("Usage `.host <name> <positions>`").then((amsg) => {
            setTimeout(() => {
                amsg.delete();
            }, 2000);    
        });
    ...
    msg.channel.send(createEmbed(games[name])).then((msg) => {
...
function join(msg) {
    ...
    if(tokens.length < 2) {
        msg.reply("Usage `.join <name>`").then((amsg) => {
            msg.delete();
            setTimeout(() => {
                amsg.delete();
            }, 2000);    
    ...
    if(!game) {
        msg.delete();
        msg.reply("No such game " + name).then((amsg) => {
            setTimeout(() => {
                amsg.delete();
            }, 2000);    
        });    
    ...
    for(let key in games) {
        ...
        if(g.players.includes(player)) {
            msg.delete();
            msg.reply("Already in game " + g.name).then((amsg) => {
                setTimeout(() => {
                    amsg.delete();
                }, 2000);    
            });    
    ...
    if(game.status != GAME_STATUS_WAITING) {
        msg.delete();
        msg.reply("Game has already started!").then((amsg) => {
            setTimeout(() => {
                amsg.delete();
            }, 2000);    
        });    
...
function doJoin(game, msg) {
    ...
    if(game.positions == game.players.length) {
        ...
        msg.channel.send(notify).then((amsg) => {
            game.notify = amsg;
        });
        ...
        game.timeout = setTimeout(() => {
            ...
            game.notify.channel.send(notify).then((amsg) => {
                setTimeout(() => {
                    amsg.delete();
                }, 5000);
            });
...
function quit(msg) {
    ...
    if(!game) {
        msg.delete();
        msg.reply("Not in a game").then((amsg) => {
            setTimeout(() => {
                amsg.delete();
            }, 2000);    
        });    
    ...
    if(game.status == GAME_STATUS_STARTED) {
        msg.delete();
        msg.reply("Game already started!").then((amsg) => {
            setTimeout(() => {
                amsg.delete();
            }, 2000);
        });
...
function cancel(msg) {
    ...
    if(tokens.length < 2) {
        msg.reply("Usage `.cancel <name>`").then((amsg) => {
            msg.delete();
            setTimeout(() => {
                amsg.delete();
            }, 2000);    
...
function end(msg) {
    ...
    if(tokens.length < 3 || (tokens[2] != "A" && tokens[2] != "B")) {
        msg.reply("Usage `.end <name> <team won (A/B)>`").then((amsg) => {
            msg.delete();
            setTimeout(() => {
                amsg.delete();
            }, 2000);    
...
function list(msg) {
    for(let key in games) {
        let game = games[key];
        game.originalMsg.channel.send(createEmbed(game)).then((amsg) => {
